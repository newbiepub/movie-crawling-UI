    ...
    async getMovieDetail(url) {
        try {
            ...
            if (response.statusCode === 200) {
                ...
                let movieTitle = $("h1.name").text().trim(),
                    movieImage = $(".poster img")[0].attribs.src,
                    category = $(".meta-data label:contains('Thể loại:')").next().text(),
                    year = $(".meta-data label:contains('Năm xuất bản:')").next().text(),
                    duration = $(".meta-data label:contains('Thời lượng')").next().text(),
                    numberOfEp = $(".meta-data label:contains('Đang phát:')").next().text(),
                    pubDate = "",
                    description = $(".film-content").html(),
    ...
    async getServerEpisode(url) {
        try {
            ...
            if (response.statusCode === 200) {
                 ...
                 * List Episode
                 */
                let list_eps = $("#list_episodes>li");
                if (list_eps.length) {
                    let values = _.values(list_eps),
                        episodes = [];
                    values.splice(list_eps.length, 4)
                    _.each(values, (item) => {
                        let epName = item.children[0].children[0].data,
